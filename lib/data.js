...
exports.findById = findById = function findById(id) {
	var result, index = 0, len = wines.length;
	for (; index < len; index++) {
		if (wines[index].id === id) {
			util.log("Lo encontramos!!!");
			result = wines[index];
			result._idx = index;
			break;
		}
	...
	if (result) {
		...
		throw new Error("No wine found with identifier: " + id);
	}
};

exports.findByName = function(name) {
	return wines.filter(function(item, index, array) {
		return item.name.toUpperCase().indexOf(name.toUpperCase()) !== -1;
	});
};

exports.create = function(wine) {
	wine.id = wines.length + 1;
	wines.push(wine);
	return wine;
};

exports.update = function(wine) {
	var oldWine = findById(wine.id);
	wines[oldWine._idx] = wine;
	return wine;
};

exports.remove = function(id) {
	var oldWine = findById(id);
	wines.splice(oldWine._idx, 1);
};